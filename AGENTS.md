...
`check-skills.sh` is the project's own gate and passes before anything is reported done.

Node is here for the commit tooling and nothing else. **No skill may depend on it**: a skill is
Markdown, and where it needs determinism, a script that runs with what the reader already has.
The moment a skill needs `pnpm install` to work, it has stopped being portable.
...
- **Semantic branches** — `<type>/<kebab-description>`, the type from that same list, ending
  ...
  anything else and runs the gate, so nothing leaves the machine with a malformed skill.
